...
M = 100
obstacles = [[1.75, 0.75, 0.6], [0.55, 1.5, 0.5], [0, -1, 0.25], [0,-2,0.25]]
...
def detect_collision(line_seg, circle):
    ...
    c_vec = np.array([line_seg[2][0], line_seg[2][1]])
    d_vec = np.array([circle[0], circle[1]])

    radius = circle[2]

    line_vec = b_vec - a_vec
    line_mag = np.linalg.norm(line_vec)
    circle_vec = d_vec - a_vec
    ...
    if np.linalg.norm(closest_point - d_vec) > radius:
        return False

    line_vec = c_vec - a_vec
    line_mag = np.linalg.norm(line_vec)
    circle_vec = d_vec - a_vec
    proj = circle_vec.dot(line_vec / line_mag)
    if proj <= 0:
    ...
    line_vec = b_vec - c_vec
    line_mag = np.linalg.norm(line_vec)
    circle_vec = d_vec - c_vec
    proj = circle_vec.dot(line_vec / line_mag)
    if proj <= 0:
        closest_point = c_vec
    elif proj >= line_mag:
        ...
        closest_point = c_vec + line_vec * proj / line_mag
...
class NLinkArm(object):
    ...
    def update_points(self):
        for i in range(1, self.n_links + 1):
            ...
            self.points[i][2] = self.points[i - 1][1] + \
                self.link_lengths[i - 1] * \
                np.sin(np.sum(self.joint_angles[:i]))   
